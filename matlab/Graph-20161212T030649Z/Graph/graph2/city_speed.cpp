  // cl -EHsc H:\Research\documents\CityModel\Graph\Graph2\city_speed.cpp
#include <stdio.h>
#include <conio.h>
#include <math.h>
...
void main(){
	...
	float Vx_max;  // km/h
	float Vx_min = 18;	//km/h
	float Vy_max;
	float Vy_min= 18;

	float E_u = 2;   // Umax = 4 , So E(u) = Umax/2 = 2
	float res;
	float t;
...
//t = log(Vx_max/Vx_min);
//printf("\n the value of t = %f",t);
fp = fopen("H:\\Research\\documents\\CityModel\\Graph\\graph2\\City_speedRes.txt","wt");
	if (fp == NULL){
		printf("\n Error");
		return;
	}
	...
	int i;
	fprintf(fp,"Speed   Residence times");
	for(i = 10; i<=35;i+=5){
		Vx_max = Vy_max = i* 3.6;
